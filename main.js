window.addEventListener('load', () => {
	...
	btn_get.addEventListener('click', () => {
		...
		let saved_list = JSON.parse(window.localStorage.getItem('To do'));
	...
	btn_save.addEventListener('click', () => {
		...
		if(list_group.querySelectorAll('.list-group-item span').length == 0) {
			notice.innerHTML = '';
			let error = document.createElement('p');
			error.classList.add('alert', 'alert-danger');
		...
		let list_items = [...list_group.querySelectorAll('.list-group-item span')].map((item, index) => {
